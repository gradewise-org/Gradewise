...
// Macro that defines a Restate service trait and generates an HTTP client implementation for it.
//
// Usage:
//
// ```
...
macro_rules! define_restate_service {
    (
        $(#[$meta:meta])* // Allow doc comments and attributes
            ...
            impl [<Http $service_trait Client>] {
                ...
                $(
                    pub async fn $method_name(&self, $($param_name: $param_type),*) -> Result<$return_type, crate::durable::service::rules::DurableClientError> {
                        ...
                        let url = format!("{}/{}/{}", self.base_url, service_name, stringify!($method_name));
